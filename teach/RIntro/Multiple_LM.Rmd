...
class()
```
```{r data type check answer, echo=soln, eval=FALSE}
#Use the class() function to check for the other columns: 
...
```{r set data type, echo=TRUE, eval=FALSE}
...
```{r multi-variate model practice, echo=TRUE, eval=FALSE}
multi_model<-lm(totalzoop~waterlevel, data=multi_data)
```
```{r echo=soln, results="hide"}
...
```{r model plot, echo=TRUE, eval=FALSE}
