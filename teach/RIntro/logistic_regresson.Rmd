...
example$easy_to_eat_log <- ifelse(example$easy_to_eat < 1, "yes", "no")
...
# Create the labels based on presence_numeric values
labels <- ifelse(presence_numeric_example == 0, "Hard to eat", "Easy to eat")

library(dplyr)
library(plotly)
# Create the plot using Plotly
plot_ly() %>%
  add_markers(data = example, x = ~NO2_NO3_ECCC1m, y = ~presence_numeric_example,
  type = "scatter", mode = "markers", text = ~labels, textposition = "middle right", hoverinfo = "text") %>%
  layout(title = "Comparison of edible and less edible phytoplankton",
  xaxis = list(title = "Nitrite + Nitrate (mg/L)", gridcolor = "rgba(0,0,0,0)"), yaxis = list(title = "How easy is it to eat?", range = c(-0.1, 1.1), gridcolor = "rgba(0,0,0,0)"), legend = list(orientation = "h", x = 0, y = 1), font = list(size = 16), showlegend = FALSE, plot_bgcolor = "rgba(0,0,0,0)", paper_bgcolor = "rgba(0,0,0,0)", margin = list(t = 80))
```
...
predictor_values <- seq(min(example$NO2_NO3_ECCC1m), max(example$NO2_NO3_ECCC1m), length.out = length(example$easy_to_eat_log))
...
# Create the plot using Plotly
plot_ly() %>%
  add_lines(x = predictor_values, y = predicted_probabilities_example, type = "scatter", mode = "markers", line = list(color = "black", width = 2), text = "Predicted probabilities",textposition = "middle right", hoverinfo = "text") %>%
  add_lines(x = predictor_values, y = lower_bound, type = "scatter", mode = "markers", line = list(color = "blue", width = 2, dash = "dash"), text = "Lower bound", textposition = "middle right", hoverinfo = "text") %>%
  add_lines(x = predictor_values, y = upper_bound, type = "scatter", mode = "markers", line = list(color = "blue", width = 2, dash = "dash"), text = "Upper bound", textposition = "middle right", hoverinfo = "text") %>%
  add_markers(data = example, x = ~NO2_NO3_ECCC1m, y = ~presence_numeric_example,
  type = "scatter", mode = "markers", text = ~labels, textposition = "middle right", hoverinfo = "text") %>%
  layout(title = "Comparison of edible and less edible phytoplankton",
  xaxis = list(title = "Nitrite + Nitrate (mg/L)", gridcolor = "rgba(0,0,0,0)"), yaxis = list(title = "Predicted probability", range = c(-0.1, 1.1), gridcolor = "rgba(0,0,0,0)"), legend = list(orientation = "h", x = 0, y = 1), font = list(size = 16), showlegend = FALSE, plot_bgcolor = "rgba(0,0,0,0)", paper_bgcolor = "rgba(0,0,0,0)", margin = list(t = 80))
```
...
```{r include=FALSE}
filam_diatom_P <- ham[!is.na(ham$filamentous_Diatom), ]
filam_diatom_P <- ham[!is.na(ham$`TP dissolved_ECCC1m`), ]
```
...
```{r include=FALSE}
filam_diatom_P <- subset(filam_diatom_P, (season==2 | season==3) & (Station_Acronym=="HH6"))
```
...
```{r include=FALSE}
model <- glm(filam_presence ~ Total_phosphorus,
...
library(pROC)  # For comparison with manual calculation
library(knitr)

# Define your datasets
datasets <- list(
  list(presence = presence_numeric_example, predicted = predicted_probabilities_example),
  list(presence = presence_numeric_filam, predicted = predicted_probabilities_filam),
  list(presence = presence_numeric_P, predicted = predicted_probabilities_P)
)

# Initialize a vector to store the AUC results
auc_results <- numeric(length(datasets))

# Loop through the datasets to calculate AUC
for (i in seq_along(datasets)) {
  roc_curve <- roc(datasets[[i]]$presence, datasets[[i]]$predicted)  # Generate ROC curve using pROC
  auc_results[i] <- auc(roc_curve)  # Calculate AUC using pROC
}

# Create a data frame with appropriate column names
auc_results_df <- data.frame(
  "Phyto & nitrate" = auc_results[1],
  "Diatoms & temperature" = auc_results[2],
  "Diatoms & phosphorus" = auc_results[3]
)

# Display the table using kable
knitr::kable(auc_results_df, digits = 2)
```
