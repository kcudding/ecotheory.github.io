...
```{r echo=FALSE}
knitr::kable(filam_diatom[c(25:30),],row.names = FALSE, digits=2, align=rep('c', 3), col.names = c("Location", "Season", "Total phosphorus", "Filamentous diatom biomass", "Epilimnion temperat."))
```
...
colnames(filam_diatom_P)[colnames(filam_diatom_P) == "TP dissolved_ECCC1m"] <- "Total_phosphorus"

filam_diatom_P <- filam_diatom_P[!is.na(filam_diatom_P$Total_phosphorus), ]
```
...
```{r echo=soln}
filam_diatom_P <- filam_diatom_P[(filam_diatom_P$season == 2 | filam_diatom_P$season == 3) & filam_diatom_P$area_group != "deep", ]
```
