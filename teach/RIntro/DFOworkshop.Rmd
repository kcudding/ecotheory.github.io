...
We can create new data subsets by using the **c()** or **c**ombine function. 

#### Try it now:
Let's access the first 3 and column 10, and save the result to a new variable ' ham_sub'
...
#### Try it now: 
Access the Station_Acronym column of the data
...
#### Try it now:
You should see that the funciton **mean()** works on a vector of numeric data.
...
Kind of hard to read... I think I wan to group the data by decade, and let's pimp the plot a little.  We also need axis labels, a nicer symbol and a legend. 


## The Editor window

Preveiously you could have completed all of these commands in the Console window where R would execute them immediately. Now we're going to have a bunch of commands, some of which we may want to tweek (e.g., colours). This time, let's save our code in the Editor window so that it is easy to change and rerun.
...
Finally, let's complete a couple of simple tests.
