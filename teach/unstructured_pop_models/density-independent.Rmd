...
* Develop practical skills to independently run a model using their own data based on provided theory and codes
* Assess the results of population models to analyze a problem of their interest
* Distinguish and compare several models and select the most appropriate model according to life histories and data available 
...
![Thomas Malthus by Elhuyar Fundazioa (https://commons.wikimedia.org/wiki/File:Malthus.png), Creative Commons License CC-BY-SA-3.0](https://raw.github.com/kcudding/kcudding.github.io/main/teach/unstructured_pop_models/malthus.png) 
...
![Common knapweed (*Centaurea nigra*) by Kate Jewell (https://commons.wikimedia.org/wiki/File:Common_knapweed_%28Centaurea_nigra%29_-_geograph.org.uk_-_492672.jpg), Creative Commons License CC-BY-SA-2.0](https://raw.github.com/kcudding/kcudding.github.io/main/teach/unstructured_pop_models/knapweed.jpg)

* **Management of species at risk of extinction**: Well-managed species will have little limitation to growth due to abundant resources and small population sizes, resulting in exponential growth rates. This was observed following management strategies to recover endangered crane populations in both North America and Japan. Whooping cranes (*Grus americana*) are the largest North American birds which became severely endangered with a population of 21 birds in 1952, as well as the red-crowned crane of Hokkaido (*Grus japonensis*) in Japan, with 33 individuals at this same year. Since then, management efforts such as federal protection and creating wildlife refuges ensured an exponential growth to bird populations, achieving 506 North American cranes and 1,900 Japanese cranes in 2020. The much faster rate at which Japanese cranes are growing is attributed to an additional management strategy consisting of artificially feeding birds during periods of food scarcity which was not implemented in North America (Gronewold, 2021).

![Endangered cranes: Whooping crane to the left and red-crowned crane to the right, by  CheepShot (https://commons.wikimedia.org/wiki/File:Whooping_Crane_%283873713594%29.jpg), Creative Commons License CC-BY-2.0, and Peter Batty (https://www.flickr.com/photos/ebatty/50772105027), Creative Commons License CC BY-NC-SA 2.0 ](https://raw.github.com/kcudding/kcudding.github.io/main/teach/unstructured_pop_models/cranes.png)
...
![Populations of duckweeds (*Lemna minor*) grown in laboratory conditions and optimal temperature](https://raw.github.com/kcudding/kcudding.github.io/main/teach/unstructured_pop_models/duckweed_pop2.png) 

```{r}
# Let's create a dataframe using the duckweed population counts we have just obtained from the picture above:
duckweed <- data.frame(day=(c(18,20,23,25,27,30)),pop_size=(c(9,12,23,45,68,103)))
...
The average daily lambda is `r format(round(lambda, 2), nsmall = 2)`.
...
The intrinsic rate of increase is `r format(round(r, 2), nsmall = 2)`. This means that our duckweed populations are growing at an average daily rate of `r format(round((r - as.integer(r))*100, 2), nsmall = 2)`%.
...
The doubling time for our duckweeds is `r format(round(double, 2), nsmall = 2)` days.  
...
![Grazing hippos by 	Bernard Dupont (https://commons.wikimedia.org/wiki/File:Hippo_%28Hippopotamus_amphibius%29_emaciated_specimen_grazing_on_Sabie_riverbank_..._%2833321050795%29.jpg), Creative Commons License CC-BY-SA-2.0](https://raw.github.com/kcudding/kcudding.github.io/main/teach/unstructured_pop_models/hippo2.jpg)
...
The average yearly lambda is `r format(round(lambda, 2), nsmall = 2)`.
...
The intrinsic rate of increase is `r format(round(r, 2), nsmall = 2)`. This means that this invasive hippo population is growing at an average yearly rate of `r format(round((r - as.integer(r))*100, 2), nsmall = 2)`"%.
